    ...
    public class Flac : IAudioFile
    {
        ...
        public Flac(string path)
        {
            try
            {
                ...
                {
                    // Skip ID3v2 tag
                    int tmpID3v2TagSize = Tagging.ID3v2.ID3v2Tag.GetTagSize(stream);
                    int tmpID3v1TagSize = Tagging.ID3v1.ID3v1Tag.GetTagSize(stream);
                    int tmpAPEv2TagSize = Tagging.APEv2.APEv2Tag.GetTagSize(stream);
